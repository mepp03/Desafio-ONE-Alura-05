...
public class JanelaPegarValorTemperatura extends JFrame implements ActionListener {

	private static final long serialVersionUID = -4878649818741078005L;
	JButton botaoOk, botaoCancelar;
	JTextField input;

	JanelaPegarValorTemperatura() {
	...
	public void actionPerformed(ActionEvent e) {
		...
		if (e.getSource() == botaoOk) {
			if (input.getText().isBlank() || input.getText().contentEquals(".") || !input.getText().matches(regex)) {
				...
				this.dispose();
				new JanelaConversorDeTemperatura(input.getText());
			}
		}
