...
public class JanelaPegarValorVelocidade extends JFrame implements ActionListener {

	private static final long serialVersionUID = 1532399555516452553L;
	JButton botaoOk, botaoCancelar;
	JTextField input;

	JanelaPegarValorVelocidade() {
	...
	public void actionPerformed(ActionEvent e) {
		...
		if (e.getSource() == botaoOk) {
			if (input.getText().isBlank() || input.getText().contentEquals(".") || !input.getText().matches(regex)) {
				...
				this.dispose();
				new JanelaConversorDeVelocidade(input.getText());
			}
		}
